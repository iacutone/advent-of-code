defmodule Day6 do
  ...
  """
  def parse_coordinate(string) when is_binary(string) do
  ...
  def largest_finite_area(coordinates) do
    ...
    {_coordinate, count} = 
      Enum.max_by(finite_count, fn {_coordinate, count} -> count end)

    count
  end

  def points_within_maximum_total_distance(coordinates, maximum_distance) do
    {x_range, y_range} = bounding_box(coordinates)

    for x <- x_range, 
        y <- y_range,
        point = {x, y},
        sum_coordinates(coordinates, point) < maximum_distance,
        do: point
  end

  defp sum_coordinates(coordinates, point) do
    coordinates
